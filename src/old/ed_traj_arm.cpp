...
FollowJointTrajectoryClient::FollowJointTrajectoryClient() :
    traj_client_("/arm_controller/follow_joint_trajectory", true), got_joint_state_(false), spinner_(0)
{
...
{// std::cout<<" Entrada "<<std::endl;
 ...
 //  std::cout<<goal<<" puntos "<<sizeP<<std::endl;
  for (int i=0;i<sizeP;i++)
  {
      ...
      goal.trajectory.points[i].positions[6]= -goal.trajectory.points[i].positions[1];
...
control_msgs::FollowJointTrajectoryGoal FollowJointTrajectoryClient::makeArmUpTrajectory(std::vector<double> joints_obj)
{
  ...
  trajectory_msgs::JointTrajectory trajectory;https:
...
for (int i=0;i<6;i++){
   ...
   if(tempdiff1>tempdiff) { //escoger la diferencia mas grande excepto en el 6to link que no tiene gran aporte al movimiento total; esta bien, link 7 es igual a link -1
       if (i==5)   {tempdiff=tempdiff1/1.5;std::cout<<" -----------ANGULO ULTIMO LINK  "<<tempdiff<<std::endl;}
       else{   tempdiff=tempdiff1;}
   }
}
promDiff=(tempdiff*5)/(5*PI); //esta seria la diferencia mas grande entre links
...
//
  ...
  // trajectory point:
  //ind++;
  if (promDiff>0.2&&tempdiff1<2) { //no deberia ser tempdiff en vez de tempdiff1??? tal vez porque tambien hay que dejar tiempo en caso de que el end effector haga una rotacion larga
    promDiff=promDiff*2; //incrementar mucho en caso de que la diferencia sea grande. para dar mas tiempo al robot para llegar
    }

  double Ttimer=0.0+(promDiff*M1);
  trajectory.points[ind].time_from_start = ros::Duration(Ttimer);//tiempo mejorado, que depende de la diferencia de distancia entre joints
  trajectory.points[ind].positions.resize(NUM_JOINTS);
  trajectory.points[ind].positions = mid_positions;//posiciones finales, no medias
...
int main(int argc, char** argv)
{
   ...
   goaled.trajectory = my_plan.trajectory_.joint_trajectory;
    ...
    //  std::out<<" POSE X: "<<  currentPose.position.x<<" POSE Y: "<<  currentPose.position.y<<" POSE Z: "<<  currentPose.position.z<<" ORI X: "<<  currentPose.orientation.x<<" ORI Y: "<<  currentPose.orientation.y<<" ORI Z: "<<  currentPose.orientation.z<<" ORI W: "<<  currentPose.orientation.z<<std::endl;
  ...
  //  success = group.plan(my_plan);
    ...
    float xo=0.0,sx=0,sy=0, yo=0.0,x0=0, y0=0,x1=0,x2=0,x3=0,x4=0,y1=0,y2=0,y3=0,x5=0,y4=0,y5=0,max=0.18;
    while (ros::ok())
    {
        ...
        if (state_ac>1){
            ...
            float yc1 = (markpose.position.x) * sin(poseCurrent.z) +  (markpose.position.y) * cos (poseCurrent.z);
            float xc1 = (markpose.position.x) * cos(poseCurrent.z) -  (markpose.position.y) * sin (poseCurrent.z);
            if (cont3>1){
                cont3=0;
           ...
           // std::cout<<target_pose3.position.x<<" '''''''''' "<<target_pose3.position.y<<std::endl;
            ...
            if (  target_pose3.position.x >= max) target_pose3.position.x = max;//limitaciones rectangulares
            if (  target_pose3.position.x <= -max) target_pose3.position.x = -max;
            if (  target_pose3.position.y >= max) target_pose3.position.y = max;
            if (  target_pose3.position.y <= -max) target_pose3.position.y = -max;
            ...
            if (rad>=0.18){//Circulo externo
                ...
                target_pose3.position.y=0.1*sin(theta);
            ...
            if (rad<=radint){  //Circulo interno
               ...
               double corrx=0.08*(xf3-currentPose.position.x);
               double corry=0.08*(yf3-currentPose.position.y);
           ...
           /* cont3=(2*PI/puntosC)*(count1 % puntosC);
            sx=(0.5*radio)*cos(cont3)+ centerx;
            sy=(1.2*radio)*sin(cont3)+ centery;

            if (count1 % 10== 0) mul=mul*(-1);
            if (count1 % 5== 0) mu2=mu2*(-1);
            std::vector<double> jvh;

            // target_pose1.position.y += mul*0.01;//0.1
            //  target_pose1.position.x += mu2*0.001;//0.1
            target_pose1.position.y = sy;//0.1
            target_pose1.position.x = sx;//0.1*/
