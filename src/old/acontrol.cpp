...
float key_x,key_az,key_y,key_z,range,GPS,landing_flag,height;
...
// ros::Publisher pub3=n.advertise<geometry_msgs::Twist>("/summit_xl_control/cmd_vel", 1);//robot2/cmd_vel
  ...
  ros::Subscriber submark = n.subscribe("/tag_detections_pose", 1, Mark_Handler);  //Marker pose
  ...
  while (ros::ok())
  {
      ...
      if (state != 1)
      {
          Kp = 0.4;//se mueve sin que haya movimiento del brazo
          sv5=0;
          sv4=0;
          sv3=0;
          sv2=0;
          ...
          sv1=summit_vel;
          svp=(sv1+sv2+sv3)/3;
          Kp = 0.5 + svp*1.4;// Constante de control de posicion del Quad
       //   std::cout<<"//Constante CCCC: "<<Kp<<" Vel: "<<summit_vel<<std::endl;
          }
        else   {
              svp=0;Kp = 0.25;
                if (contvel > 1) Kp = 0.3;
     ...
     // std::cout<<"//EdTrackingState: "<<EdTrackingState<<" Dff: "<<diff<<std::endl;
      if (state!=1){
        height= GPS-0.15;
         //std::cout<<"//GGGGGGGGGGGGGG"<<height<<std::endl;
        cont1=0;
      }
      else{
          cont1++;
          if (cont1>4){
          if((1*markpose.position.z) >= 0.5) { heighmark = 0.5/1.55; }
          else
           {heighmark =markpose.position.z;}
          height= eeff_pose.position.z + (2*heighmark) +0.11;
         // height= GPS+0.075;
         // std::cout<<"//AAAAAAAAAAAAAaa "<<heighmark<<std::endl;
          }
          else
              {height= GPS-0.15;}
      }
      ...
      else
      {
          ...
          error=1.49-height;//1.33
          if (error>1) error=1;
          if (error<-1) error=-1;
//Kp=0;
          ed_control.linear.x = key_x;//1.1*Kp * xerr + key_x;
          ed_control.linear.y = key_y;//Kp * yerr + key_y;
          ed_control.linear.z = key_z+(error*0.35);
          ed_control.angular.y= 0.0;
          ed_control.angular.x= 0.0;
          ed_control.angular.z= key_az;
          if (cont<55) ed_control.linear.z = 0.005;
      }
